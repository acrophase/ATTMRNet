...
for item in enumerate(data.keys()):
    ...
    for i in range(len(edr_hrv)):
        edr_hrv[i] = signal.resample(edr_hrv[i] , 128)
        edr_rpeak[i] = signal.resample(edr_rpeak[i] , 128)
        adr[i] = signal.resample(adr[i] , 128)
        ref_resp[i] = signal.resample(ref_resp[i] , 128)
