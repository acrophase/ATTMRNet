...
parser.add_argument(
    "--save_model_path", type=str, help="Path to saved model", default='/media/acrophase/pose1/charan/BR_Uncertainty/BRUCE_KNOWLEDGE_DISTILL/BRUCE_SAVED_STUDENT_MODEL'
...
for item in config_list:
    if item == "four":
        ...
        model  = BRUnet_Multi_resp_student_3(model_input_shape)
        loss_fn = Huber()
        save_path = '/media/acrophase/pose1/charan/BR_Uncertainty/BRUCE_KNOWLEDGE_DISTILL/BRUCE_SAVED_STUDENT_MODEL_CONFIG'
        ...
        for epoch in tqdm.tqdm(range(num_epochs)):
            ...
            for step, (x_batch_train , y_batch_train, x_batch_train_ref_rr) in enumerate(train_dataset):
                with tf.GradientTape() as tape:
                    output, out_rr,_,_,_,_,_,_,_,_ = model(x_batch_train , training = True)
            ...
            for step , (x_batch_test,y_batch_test,x_batch_test_ref_rr) in enumerate(test_dataset):
                test_output,test_out_rr,_,_,_,_,_,_,_,_ = model(x_batch_test)
            ...
            if mean_loss < best_loss:
                ...
                model.save_weights(os.path.join(results_path, 'best_model_'+str(1e-2)+'_'+str(1e-3)+'_'+str(num_epochs)+'.h5'))
    ...
    if item == "two":
        def scheduler (epoch):
            ...
            return lr
        model_input_shape = (128,3)
        model  = BRUnet_Multi_resp_student_4(model_input_shape)
        loss_fn = Huber()
        save_path = '/media/acrophase/pose1/charan/BR_Uncertainty/BRUCE_KNOWLEDGE_DISTILL/BRUCE_SAVED_STUDENT_MODEL_CONFIG'
        results_path = os.path.join(save_path , item.lower())
        if not(os.path.isdir(results_path)):
        ...
        for epoch in tqdm.tqdm(range(num_epochs)):
            ...
            for step, (x_batch_train , y_batch_train, x_batch_train_ref_rr) in enumerate(train_dataset):
                with tf.GradientTape() as tape:
                    output, out_rr,_,_,_,_,_,_ = model(x_batch_train , training = True)
                    loss_value = loss_fn(y_batch_train , output)
                    loss_value_rr = loss_fn(x_batch_train_ref_rr, out_rr)
                    net_loss_value = loss_value + loss_value_rr
                    train_loss_list.append(net_loss_value)  
            ...
            for step , (x_batch_test,y_batch_test,x_batch_test_ref_rr) in enumerate(test_dataset):
                test_output,test_out_rr,_,_,_,_,_,_ = model(x_batch_test)
                test_loss_resp = loss_fn(y_batch_test , test_output)
                test_loss_rr = loss_fn(x_batch_test_ref_rr , test_out_rr)
                test_loss_val = test_loss_resp + test_loss_rr
                test_loss(test_loss_val)
    ...
    if item == "one":
        ...
        for epoch in tqdm.tqdm(range(num_epochs)):
            ...
            for step, (x_batch_train , y_batch_train, x_batch_train_ref_rr) in enumerate(train_dataset):
                with tf.GradientTape() as tape:
                    output, out_rr,_,_,_,_,_ = model(x_batch_train , training = True)
                    loss_value = loss_fn(y_batch_train , output)
                    loss_value_rr = loss_fn(x_batch_train_ref_rr, out_rr)
                    net_loss_value = loss_value + loss_value_rr
                    train_loss_list.append(net_loss_value)  
            ...
            for step , (x_batch_test,y_batch_test,x_batch_test_ref_rr) in enumerate(test_dataset):
                test_output,test_out_rr,_,_,_,_,_ = model(x_batch_test)
                test_loss_resp = loss_fn(y_batch_test , test_output)
                test_loss_rr = loss_fn(x_batch_test_ref_rr , test_out_rr)
                test_loss_val = test_loss_resp + test_loss_rr
                test_loss(test_loss_val)
