...
parser.add_argument(
    "--save_model_path", type=str, help="Path to saved model", default='/media/acrophase/pose1/charan/BR_Uncertainty/BRUCE_KNOWLEDGE_DISTILL/BRUCE_SAVED_STU_GND'
...
model_st.load_weights('/media/acrophase/pose1/charan/BR_Uncertainty/BRUCE_KNOWLEDGE_DISTILL/BRUCE_SAVED_TEACH_KD_STU_MODEL/TEAC_KD_STU_MODEL/best_model_60.01_0.001_100.h5')
save_path = (args.save_model_path)
results_path = os.path.join(save_path , "STU_GND_MODEL")
...
train_log_dir = 'evi_stu_gnd/logs/gradient_tape/'+ current_time + '/train'
test_log_dir = 'evi_stu_gnd/logs/gradient_tape/' + current_time + '/test'
...
for epoch in tqdm.tqdm(range(num_epochs)):
    ...
    for step, (x_batch_train , y_batch_train, x_batch_train_ref_rr) in enumerate(train_dataset):
        with tf.GradientTape() as tape:
            out_st,out_rr_st,e6_st,at1_st,at2_st,at4_st,at5_st,at6_st,at7_st= model_st(x_batch_train , training = True)
            ...
            loss_value_st = loss_fn(y_batch_train , out_st)
            loss_value_rr_st = loss_fn(x_batch_train_ref_rr, out_rr_st)

            loss_value_st_teac = loss_fn(out_tea,out_st)
            loss_value_rr_st_teac = loss_fn(out_rr_tea , out_rr_st)

            net_loss_value_st_gnd = loss_value_st + loss_value_rr_st
            net_loss_st_teach = loss_value_rr_st_teac + loss_value_st_teac

            net_loss_value = alpha*net_loss_value_st_gnd + (1-alpha)*net_loss_st_teach
            train_loss_list.append(net_loss_value)  
        grads = tape.gradient(net_loss_value, model_st.trainable_weights)
        optimizer.apply_gradients(zip(grads, model_st.trainable_weights)) 
    ...
    test_loss_list = []
    best_loss = 100000
    print(alpha*net_loss_value_st_gnd)
    print((1-alpha)*net_loss_st_teach)
    for step , (x_batch_test,y_batch_test,x_batch_test_ref_rr) in enumerate(test_dataset):
        test_out_st,test_out_rr_st,test_e6_st,test_at1_st,test_at2_st,test_at4_st,test_at5_st,\
                                               test_at6_st,test_at7_st= model_st(x_batch_test , training = True)
        ...
        test_loss_resp_st = loss_fn(y_batch_test , test_out_st)
        test_loss_rr_st = loss_fn(x_batch_test_ref_rr , test_out_rr_st)
        
        test_loss_val_st = test_loss_resp_st + test_loss_rr_st
        
        test_loss_resp_st_teach = loss_fn(test_out_tea , test_out_st)
        test_loss_rr_st_teach = loss_fn(test_out_rr_tea,test_out_rr_st)
        
        test_loss_val_st_teach = test_loss_resp_st_teach + test_loss_rr_st_teach

        test_loss_val = alpha*test_loss_val_st + (1-alpha)*test_loss_val_st_teach
